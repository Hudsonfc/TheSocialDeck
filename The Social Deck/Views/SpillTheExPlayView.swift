//
//  SpillTheExPlayView.swift
//  The Social Deck
//

import SwiftUI

struct SpillTheExPlayView: View {
    @ObservedObject var manager: SpillTheExGameManager
    let deck: Deck
            ...
            VStack(spacing: 0) {
                        ...
                        (swipeNavigationEnabled && (roomId == nil || isHost)) ? DragGesture()
                            ...
                            .onEnded { value in
                                if !isTransitioning {
                                    ...
                                    if value.translation.width > threshold && manager.canGoBack {
                                        previousCard()
                                    } else if value.translation.width < -threshold && !manager.isFinished {
                                        nextCard()
                                    }
                                    dragOffset = 0
                                }
                ...
                if manager.isFlipped {
                    if roomId != nil && !isHost {
                        ...
                        Text("Swipe left to go to next card or right to go to previous card")
                            .font(.system(size: 14, weight: .regular, design: .rounded))
                            .foregroundColor(Color(red: 0x7A/255.0, green: 0x7A/255.0, blue: 0x7A/255.0))
                            .multilineTextAlignment(.center)
                            .padding(.horizontal, 40)
...
struct SpillTheExCardBackView: View {
    let text: String
