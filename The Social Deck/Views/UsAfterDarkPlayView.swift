...
struct UsAfterDarkPlayView: View {
    @ObservedObject var manager: UsAfterDarkGameManager
    let deck: Deck
    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                ...
                if let currentCard = manager.currentCard() {
                    ZStack {
                        ...
                        UsAfterDarkCardFrontView()
                            .opacity(cardRotation < 90 ? 1 : 0)
                        
                        // Card back - visible when rotation >= 90, pre-rotated 180
                        UsAfterDarkCardBackView(text: currentCard.text)
                            .opacity(cardRotation >= 90 ? 1 : 0)
        ...
        .background(
            Group {
                NavigationLink(
                    destination: UsAfterDarkEndView(deck: deck, selectedCategories: selectedCategories, cardsPlayed: manager.cards.count),
...
struct UsAfterDarkCardFrontView: View {
    var body: some View {
        ZStack {
...
struct UsAfterDarkCardBackView: View {
    let text: String
...
#Preview {
    NavigationView {
        UsAfterDarkPlayView(
            manager: UsAfterDarkGameManager(
                deck: Deck(
                    title: "Us After Dark",
                    description: "Test",
                    numberOfCards: 3,
